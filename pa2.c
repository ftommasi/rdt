...

{
  ...
  int i;
  for(i=0; i < 20; i++){
    checksum_result+= (int)payload[i];//take integer value of every character
  }

  return checksum_result;
}

//functions for debugging purposes. Enable by setting DEBUG flag to 1
void dumpA(){
  printf("-----------------------A DUMP--------------------------------\n");
  int i;
  ...
  struct msg message;
{
  ...
  //buffer all incoming messages as packets
  ...
  //only send packets when the window is ready
  if(A_ready_to_send){
    if(!A_timer_set){
  ...
  struct pkt packet;
{
  ...
  if(packet.checksum == calculate_checksum(packet.seqnum,packet.acknum,packet.payload) ){
    ...
    //slide window by moving the base and end pointers by the delta from the current base to the next unacked packet
    ...
    //for acked packets update its time by taking the difference between the current time and the time it was sent
    for(i=A_window_base; i != A_window_end && i < A_next_buffer_index; i = (i+1) % BUFFER_SIZE){
      if(packet.seqnum == A_buffer[i].seqnum && !time_calc[i]){
        time_calc[i] = 1;
        A_packet_timers[i] = time_now - A_packet_timers[i];
      } 
      //cumulative ack. Mark all packets with smaller or equal acknums as acked
...
A_init (void)
{
  ...
  time_calc = (bool*) malloc(BUFFER_SIZE * sizeof(bool));  
  eval_times= (bool*) malloc(BUFFER_SIZE * sizeof(bool));  
  
  //set dummy packets for initial values of buffers
  struct pkt dummy;
  dummy.seqnum = -1;
  dummy.acknum = -1;
  ...
  struct pkt packet;
{
  ...
  if(packet.checksum == calculate_checksum(packet.seqnum, packet.acknum, packet.payload)){
    ...
    for(i = B_window_base; i != B_window_end ; i = (i+1) % BUFFER_SIZE){
      //detect if packets are out of window based on seq num
    ...
    //make sure that at least one packet has been inserted as to not run into issues with window stuff
    if(!B_inserted){
      B_inserted = 1;
      duplicate = 0;
      in_window = 1;
    }
    //this is a new packet that we are expecting and haven't acked yet
    if(!duplicate && in_window){    
      ...
      B_packet_timers[B_next_buffer_index] = time_now;//keep track of what time we got the packet
      B_buffer_acks[B_next_buffer_index] = 1;//buffer packet to detect duplicates
      B_next_buffer_index++;
      
      //keep track of what the seqnum of next expected packet is
      for(i = B_window_base; i != B_window_end ; i = (i+1) % BUFFER_SIZE){
    ...
    memcpy(ack_packet.payload,"ACK\0",4); //Ack packets have a payload of ACK
    ...
    if(B_buffer_acks[B_window_base]){
      ...
      //write to layer 5 as packet slide out of window so that everything is written in order
  ...
  void
    B_init (void)
    {
      ...
      B_packet_timers = (double*) malloc(BUFFER_SIZE * sizeof(double)); 
      B_inserted = 0; 
      
      //set dummy packts for B buffer
      struct pkt dummy;
      dummy.seqnum = -1;
      dummy.acknum = -1;
      memcpy(dummy.payload,"-------------------\n",20);
  ...
  {
    //perform final calculations and print statistical data 
    double avg_rtt_calc = 0.0;
    int walk =0;
    printf("----------------------------STATS------------------------------\n");
    int i;
    for(i=0; i < B_curr_seqno; i++){
